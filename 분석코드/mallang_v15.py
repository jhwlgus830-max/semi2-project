...
def make_wordcloud_png(texts:list):
    ...
    - XPN 결합이 적용된 kiwi_tokenize() 사용 → 접두사+명사 자동 결합
    - 적응형 빈도 필터: 누적 토큰 30개 미만이면 1회 이상 모두 표시,
      30개 이상이면 2회 이상만 표시 → 소량 대화에서도 다양한 단어 보여줌
    """
    ...
    # 적응형 빈도 필터: 코퍼스가 충분하면 2회 이상, 아니면 1회 이상 표시
    # 근거: 대화 8회 미만 소량 데이터에서는 cnt>=2 조건이 너무 가혹함
    raw_freq = Counter(tokens)
    min_freq = 2 if len(tokens) >= 30 else 1
    freq = {word: cnt for word, cnt in raw_freq.items() if cnt >= min_freq}
    if not freq: return None, unique_count
    try:
        wc = WordCloud(
            font_path=_find_ko_font(), width=700, height=420,
            background_color="white", colormap="PuBu",
            prefer_horizontal=0.9, relative_scaling=0.2, max_words=50,
            ...
            if not _WORDCLOUD_OK:
                ...
                if wc_bytes and uniq_count >= MIN_CUMULATIVE_WORDS:
                    ...
                    st.info(
                        ...
                        f"**워드클라우드 표시 조건**\n"
                        f"- ✅ 대화 횟수: {n_user_turns}회 / 최소 {MIN_TURNS_FOR_WORDCLOUD}회\n"
                        f"- {'✅' if n_valid >= MIN_TURNS_FOR_WORDCLOUD else '⏳'} "
                        f"3단어 이상 의미 있는 발화: {n_valid}회 / 최소 {MIN_TURNS_FOR_WORDCLOUD}회"
                        f"{f' (앞으로 {remain_valid}회 더 필요)' if remain_valid > 0 else ''}\n\n"
                        f"_💡 3단어 이상으로 이야기해 주시면 더 정확한 워드클라우드를 만들 수 있어요!_"
                    )
